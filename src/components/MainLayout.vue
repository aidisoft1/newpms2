...
:deep(.ant-menu-light .ant-menu-submenu-selected) {
...
export default defineComponent({
  ...
  const menuItems: MenuItem[] = [
        ...
        children: [
              {
                ...
                children: [
                  ...
                  { key: 'finance.charging.feeAdjust', label: '费用调整', route: '/finance/charging/feeAdjust' },
                  { key: 'finance.charging.notice', label: '收费通知单' },
                  { key: 'finance.charging.collectFee', label: '费用收取', route: '/finance/charging/collectFee' },
                  { key: 'finance.charging.temporaryCollection', label: '临时收款', route: '/finance/charging/temporaryCollection' }
                ]
              },
          {
            key: 'finance.budget',
      ...
      },
      {
        ...
        label: '客户服务',
        children: [
          { key: 'service.repairs.daily', label: '日常报修处理', route: '/service/repairs/daily' },
          { key: 'service.public', label: '公共服务咨询', route: '/service/public' },
          { key: 'service.emergency', label: '紧急事件响应', route: '/service/emergency' }
        ]
      },
      {
        key: 'wechat',
      ...
      { key: 'datahub', label: '数据中枢', children: [
      ...
      },
      {
        ...
        children: [
          ...
          { key: 'app.inspection', label: '巡检管管理' },
    ...
    const tabs = ref<Tab[]>([
      ...
      { key: 'finance.charging.temporaryCollection', title: '临时收款', component: shallowRef(defineAsyncComponent(() => import('../views/finance/charging/temporaryCollection.vue'))) }
    ])
  ...
  let comp: Component | any = PlaceholderView
        ...
        } else if (normalized === 'finance/charging/collectFee') {
          comp = defineAsyncComponent(() => import('../views/finance/charging/collectFee.vue'))
        ...
        } else if (normalized === 'service/repairs/daily' || normalized === 'service/repair' || normalized === 'service/repairs') {
          comp = defineAsyncComponent(() => import('../views/repairs.daily.vue'))
        } else if (normalized) {
          // 如果 normalized 中含斜杠（例如 customers/cert），尝试把斜杠替换为点（customers.cert）再导入
  ...
  if (item.route) {
        // 保持 URL 同步 — normalize to start with '/'
        const routeTo = item.route.startsWith('/') ? item.route : ('/' + item.route)
        message.info(`导航: ${item.label}`)
        console.log('[MainLayout] onMenuClick routeTo=', routeTo, 'item=', item)
  router.push(routeTo).catch((e) => { console.warn('router.push failed', e); console.error('router.push failed stack', e && e.stack) })
        // 强制为特定路径直接加载组件，避免 openTab 的路径解析问题
        if (routeTo === '/service/repairs/daily' || item.key === 'service.repairs.daily') {
          try {
            const comp = defineAsyncComponent(() => import('../views/repairs.daily.vue'))
            const tabKey = 'service.repairs.daily'
            if (!tabs.value.find((t: Tab) => t.key === tabKey)) {
              tabs.value.push({ key: tabKey, title: item.label, component: shallowRef(comp) })
            }
            activeKey.value = tabKey
            return
          } catch (err) {
            console.warn('failed to open repairs.daily tab', err)
            try { const eAny: any = err; console.error('detailed', eAny && eAny.stack ? eAny.stack : JSON.stringify(eAny)) } catch (e) { console.error('error serializing err', e) }
          }
        }
        // 同时以标签页方式打开组件（openTab 内会根据 routePath 加载正确的组件）
        openTab(item.key, item.label, routeTo)
        return
      }
      const routePath = '/' + item.key.replace(/\./g, '/')
      message.info(`导航: ${item.label}`)
      console.log('[MainLayout] onMenuClick fallback routePath=', routePath, 'item=', item)
  ...
  // 通过 provide 暴露给子组件调用，使分页签内页面也能请求全屏（优先渲染标签页组件）
...
/* Brand font: Product Sans (local file required). If you don't have Product Sans,
   put the font file at src/assets/fonts/ProductSans-Regular.ttf or change the src URL
   to your licensed font. Fallbacks are provided. */
@font-face {
  font-family: 'Product Sans';
