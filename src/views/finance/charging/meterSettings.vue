...
const metersData = ref<any[]>([
  { meterNo: 'M0001', meterName: '水表-1', meterType: '水表', roomId: 'R001', customerId: 'C001', roomName: '101室', multiple: 1, enabled: true, remark: '', startDate: null, reading: 12.5, chargeItem: '水费', formulaName: '公式A', unitPrice: 2.5 },
  { meterNo: 'M0002', meterName: '电表-1', meterType: '电表', roomId: 'R002', customerId: 'C002', roomName: '201室', multiple: 1, enabled: true, remark: '', startDate: null, reading: 45.8, chargeItem: '电费', formulaName: '公式B', unitPrice: 0.8 }
])

// 树数据（area -> building -> room）
const areaBuildings = {
  '管理区A': ['楼宇A1'],
  '管理区B': ['楼宇B1']
}
...
function buildTree() {
  ...
  for (const [area, builds] of Object.entries(areaBuildings)) {
    // each building initially has a placeholder child; real rooms are loaded on expand
...
async function loadTreeData(node: any) {
  ...
  // load rooms for a building node
...
function saveMeter() {
  ...
  if (mult == null || isNaN(Number(mult)) || Number(mult) < 1 || !Number.isInteger(Number(mult))) {
    message.error('倍率必须为大于等于 1 的整数')
    return
  }
...
function deleteMeter(r:any) { Modal.confirm({ title: '确认删除该仪表吗？', onOk() { metersData.value = metersData.value.filter((x:any)=>x.meterNo !== r.meterNo); message.success('已删除') } }) }
...
function onTableSearch() { /* computed 会响应 */ }
