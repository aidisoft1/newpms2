<template>
  <div class="public-consult-page">
    <div class="toolbar" style="display:flex; gap:8px; align-items:center; margin-bottom:12px;">
      <a-button type="primary" size="small" @click="createNewConsult">新增咨询</a-button>
      ...
      <a-button size="small" @click="importCsv">导入CSV</a-button>
              ...
              <!-- 编辑 Modal -->
              <a-modal v-model:visible="editModalVisible" title="编辑咨询" :width="720" :bodyStyle="{ maxHeight: '70vh', overflow: 'auto', padding: '16px 20px' }" @ok="saveEditModal" @cancel="closeEditModal" :destroyOnClose="true" @afterClose="onEditModalAfterClose" centered>
              ...
              <!-- 管理咨询分类 Modal -->
              <a-modal v-model:visible="manageServiceVisible" title="管理咨询分类" @ok="saveManageService" @cancel="manageServiceVisible=false">
    ...
    <div v-else>
      <a-table :columns="antColumns" :data-source="tableData" row-key="id" size="small" :pagination="{ pageSize: 20 }" :scroll="{ x: scrollX }" @change="onTableChange"></a-table>
          ...
          const allFields = ref([
            ...
            { key: 'serviceCategory', title: '咨询分类', visible: true, groupable: true },
            { key: 'appointmentTime', title: '预约上门时间', visible: true, groupable: false },
            { key: 'description', title: '处理说明', visible: true, groupable: false },
            { key: 'visitStartTime', title: '处理开始时间', visible: true, groupable: false },
            { key: 'repairEndTime', title: '处理结束时间', visible: true, groupable: false },
            { key: 'completionImages', title: '图片', visible: true, groupable: false },
          ...
          for (let i = 1; i <= 24; i++) {
            consultData.value.push({
              ...
              customerId: 'C' + (200 + i),
              customerName: '客户' + i,
              phone: '1381000' + String(200 + i),
              priority: i % 3 === 0 ? '高' : (i % 3 === 1 ? '中' : '低'),
              serviceCategory: i % 2 === 0 ? '物业咨询' : '费用咨询',
              appointmentTime: '',
              description: i % 2 === 0 ? '在线咨询' : '电话咨询',
              visitStartTime: '',
              repairEndTime: '',
              completionImages: [],
              orderCloseTime: ''
          ...
          function statusColor(s: string) { switch (s) { case '新建': return 'blue'; case '处理中': return 'orange'; case '待确认': return 'gold'; case '已解决': return 'green'; case '已关闭': return 'gray'; default: return 'default' } }
