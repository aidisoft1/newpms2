<template>
  <div class="repairs-page">
    <div class="toolbar" style="display:flex; gap:8px; align-items:center; margin-bottom:12px;">
      <a-button type="primary" size="small" @click="createNewOrder">新增订单</a-button>
  ...
  <div v-if="groupField">
      ...
      <div v-show="!collapsedGroups[g]">
              <a-table row-key="id" :key="tableKey" :columns="antColumns" :data-source="rowsWithIndex(rows)" :pagination="false" size="small" :scroll="{ x: scrollX, y: '60vh' }" @change="onTableChange">
              <template #bodyCell="{ column, record, index }">
                <div v-if="column.dataIndex === 'rowIndex'">{{ record.rowIndex }}</div>
                <div v-else-if="column.key === '_actions'">
                  <div style="display:flex; gap:8px; justify-content:flex-end">
                    <a-button size="small" @click="openEditModal(record)">编辑</a-button>
                    <a-button size="small" danger @click="onDeleteRow(record)">删除</a-button>
                  </div>
                </div>
                <div v-else>
    ...
    <div v-else>
  <a-table row-key="id" :key="tableKey" :columns="antColumns" :data-source="tableData" :pagination="{ pageSize: 20 }" size="small" :scroll="{ x: scrollX, y: '60vh' }" @change="onTableChange">
        <template #bodyCell="{ column, record, index }">
          <div v-if="column.dataIndex === 'rowIndex'">{{ record.rowIndex }}</div>
          <div v-else-if="column.key === '_actions'">
            <div style="display:flex; gap:8px; justify-content:flex-end">
  ...
  <!-- debug panel removed in production -->
    ...
    <!-- 筛选 Drawer -->
    <a-drawer v-model:visible="filterVisible" title="筛选" placement="right" :width="360">
      <a-form layout="vertical">
        <a-form-item label="房间编号">
          <a-input v-model:value="filterForm.roomId" />
        </a-form-item>
        <a-form-item label="工单状态">
          <a-select v-model:value="filterForm.status" allow-clear>
        ...
        <a-form-item label="客户名称">
          <a-input v-model:value="filterForm.customerName" />
        </a-form-item>
        <a-form-item label="优先级">
          <a-select v-model:value="filterForm.priority" allow-clear>
        ...
        <div style="text-align:right; margin-top:12px">
          <a-button @click="resetFilter" style="margin-right:8px">重置</a-button>
    ...
    <!-- 编辑 Modal -->
    <a-modal v-model:visible="editModalVisible" title="编辑工单" :width="720" :bodyStyle="{ maxHeight: '70vh', overflow: 'auto', padding: '16px 20px' }" @ok="saveEditModal" @cancel="closeEditModal" :destroyOnClose="true" @afterClose="onEditModalAfterClose" centered>
      <a-form layout="vertical" class="edit-form">
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px">
          ...
          <a-form-item label="服务分类">
            <a-input size="small" v-model:value="editingRow.serviceCategory" />
          </a-form-item>
          ...
          <a-form-item label="完工图片" style="grid-column: span 2;">
            ...
            <div style="color:#888; font-size:12px; margin-top:6px">支持多图上传，图片将以 Base64 存储（仅示例，生产请接入文件服务器）。</div>
          </a-form-item>
        </div>
      </a-form>
    </a-modal>
...
// 定义列元数据
const allFields = ref([
  { key: 'status', title: '工单状态', visible: true, groupable: true },
  { key: 'roomId', title: '房间编号', visible: true, groupable: true },
  { key: 'customerId', title: '客户编号', visible: true, groupable: false },
  { key: 'customerName', title: '客户名称', visible: true, groupable: false },
  { key: 'phone', title: '联系电话', visible: true, groupable: false },
  { key: 'priority', title: '优先级', visible: true, groupable: true },
  { key: 'serviceCategory', title: '服务分类', visible: true, groupable: true },
  { key: 'appointmentTime', title: '预约上门时间', visible: true, groupable: false },
  { key: 'faultCategory', title: '故障类别', visible: true, groupable: true },
  { key: 'visitStartTime', title: '上门维修开始时间', visible: true, groupable: false },
  { key: 'repairEndTime', title: '维修结束时间', visible: true, groupable: false },
  { key: 'completionImages', title: '完工图片', visible: true, groupable: false },
  { key: 'orderCloseTime', title: '工单关闭时间', visible: true, groupable: false }
])
...
// sample rooms list and room -> customer mapping for autofill
const roomsList = ref<string[]>(repairsData.value.map(r => r.roomId))
const roomMap = reactive<Record<string, any>>({})
for (const r of repairsData.value) {
  roomMap[r.roomId] = { customerId: r.customerId, customerName: r.customerName, phone: r.phone }
}

function onRoomSelectChange(val: string) {
  if (!editingRow.value) return
  const m = roomMap[val]
  if (m) {
    // autofill but keep editable
    editingRow.value.customerId = m.customerId
    editingRow.value.customerName = m.customerName
    editingRow.value.phone = m.phone
  }
}
...
const antColumns = computed(() => {
  ...
  // actions column fixed to right for edit/delete
  cols.push({
    title: '操作',
    key: '_actions',
    fixed: 'right',
  ...
  h('a-button', { size: 'small', onClick: () => openEditModal(record) }, '编辑'),
  h('a-button', { size: 'small', danger: true, onClick: () => onDeleteRow(record) }, '删除')
      ])
    }
  })
  return cols
...
const tableData = computed(() => {
  ...
  if (sortState.columnKey) {
    ...
    const order = sortState.order // 'ascend' or 'descend'
...
function onEditRow(row: any) {
  try {
    const newName = window.prompt('编辑客户名称', row.customerName || '')
    if (newName === null) return
    row.customerName = newName
    tableKey.value = (tableKey.value || 0) + 1
    message.success('已更新')
  } catch (e) { console.warn(e) }
}

function onDeleteRow(row: any) {
  try {
    if (!confirm('确认删除该行吗？')) return
    const idx = repairsData.value.findIndex(r => r.id === row.id)
    if (idx >= 0) repairsData.value.splice(idx, 1)
    tableKey.value = (tableKey.value || 0) + 1
    message.success('已删除')
  } catch (e) { console.warn(e) }
}
...
// CSV export
function exportCsv() {
  const headers = ['序号', ...allFields.value.filter(f => f.visible).map(f => f.title)]
  const rows = tableData.value.map(r => allFields.value.filter(f => f.visible).map(f => r[f.key] ?? ''))
  const csv = [headers.join(','), ...rows.map(r => r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(','))].join('\n')
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
  const link = document.createElement('a')
  link.href = URL.createObjectURL(blob)
  link.setAttribute('download', '日常报修导出.csv')
...
function openEditModal(row: any) {
  // clone and normalize date fields for pickers
  const clone = { ...row }
  const dateFields = ['appointmentTime', 'visitStartTime', 'repairEndTime', 'orderCloseTime']
  for (const df of dateFields) {
    const v = clone[df]
    if (!v) clone[df] = undefined
    else {
      try { clone[df] = new Date(v) } catch (e) { clone[df] = undefined }
    }
  }
  // prefill completionImagesText
  if (Array.isArray(clone.completionImages)) clone.completionImagesText = JSON.stringify(clone.completionImages)
  else clone.completionImagesText = clone.completionImagesText || ''
...
function createNewOrder() {
  const newRow: any = {
    id: Date.now(),
    roomId: '', customerId: '', customerName: '', phone: '', priority: '', serviceCategory: '', appointmentTime: undefined,
    faultCategory: '', visitStartTime: undefined, repairEndTime: undefined, completionImages: [], orderCloseTime: undefined
  }
...
function saveEditModal() {
  ...
  for (const df of dateFields) {
    ...
    if (v && v.toDate) {
      // AntD moment-like
      try { payload[df] = v.toDate().toISOString() } catch (e) { payload[df] = new Date(v).toISOString() }
    } else if (v instanceof Date) payload[df] = v.toISOString()
    else if (!v) payload[df] = ''
  }
  // parse images text if present
  if (payload.completionImagesText) {
    try {
      const parsed = JSON.parse(payload.completionImagesText)
      if (Array.isArray(parsed)) payload.completionImages = parsed
      else payload.completionImages = []
    } catch (e) { payload.completionImages = [] }
    delete payload.completionImagesText
  }
  const idx = repairsData.value.findIndex(r => r.id === payload.id)
  if (idx >= 0) {
    repairsData.value[idx] = { ...repairsData.value[idx], ...payload }
  } else {
    // new record
    repairsData.value.push(payload)
  }
...
function onEditModalAfterClose() {
  // cleanup editing state
  editingRow.value = null
  // remove any possible leftover modal mask and body class (defensive)
  try {
    const masks = document.querySelectorAll('.ant-modal-mask')
    masks.forEach(m => m.parentElement && m.parentElement.removeChild(m))
    document.body.classList.remove('ant-modal-open')
  } catch (e) { console.warn('modal cleanup failed', e) }
}

// upload handlers for images (demo: store base64 in memory)
...
async function handleUpload({ file, onSuccess }: any) {
  try {
    const b64 = await fileToBase64(file)
    if (!editingRow.value) return
    if (!editingRow.value._fileList) editingRow.value._fileList = []
    const item = { uid: Date.now() + '', name: file.name, url: b64 }
    editingRow.value._fileList.push(item)
    // sync completionImages
    editingRow.value.completionImages = (editingRow.value.completionImages || []).concat([b64])
...
function handleRemove(file: any) {
  if (!editingRow.value) return
  const uid = file.uid || file.url
  editingRow.value._fileList = (editingRow.value._fileList || []).filter((f: any) => f.uid !== uid && f.url !== uid)
  editingRow.value.completionImages = (editingRow.value.completionImages || []).filter((u: any) => u !== file.url)
}
</script>
