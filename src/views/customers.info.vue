...
const tableData = ref([...customersData.value])
...
function onAdd() { editForm.value = { customerId: '', contact: '', customerName: '', customerType: '', phone: '', address: '', tel: '', fax: '', birthday: '', idType: '', idNumber: '', moveInDate: '', decorationDate: '', bankId: '', accountName: '', bankAccount: '', bankBranch: '', remark: '', buildingName: '', roomId: '' }; editModalVisible.value = true }
...
function handleEditOk() {
	const idx = customersData.value.findIndex(b => b.customerId === editForm.value.customerId)
	if (idx > -1) { customersData.value[idx] = { ...editForm.value }; filterTableData(); message.success('编辑成功') }
	else { customersData.value.push({ ...editForm.value }); filterTableData(); message.success('新增成功') }
	buildTreeFromCustomers(); refreshOptions(); editModalVisible.value = false
}
...
function onExport() {
	const headers = ['客户编号','联系人','客户名称','客户类型','手机号','通讯地址','联系电话','传真','出生年月','证件类型','身份证号','收楼日期','装修日期','银行编号','账户名称','银行账号','开户支行','备注']
	const rows = tableData.value.map(row => [row.customerId, row.contact, row.customerName, row.customerType, row.phone, row.address, row.tel, row.fax, row.birthday, row.idType, row.idNumber, row.moveInDate, row.decorationDate, row.bankId, row.accountName, row.bankAccount, row.bankBranch, row.remark])
...
function refreshOptions() {
	...
	const customerIdSet2 = Array.from(new Set(customersData.value.map(b => b.customerId)))
...
async function loadCustomers() { return Promise.resolve(customersData.value) }
